...
PASS_FILE2 = "mini_pass.json"

def load_pass():
    try:
        with open(PASS_FILE2, "r") as f:
            return json.load(f)
    except:
        return {}
...
def memory_check(q):
    ...
    if "save my password" in q or "my password is" in q:
        pwd = ""
        if "my password is" in q:
            pwd = q.split("my password is")[-1].strip()
        else:
            pwd = q.split("save my password")[-1].strip()
        if not pwd:
            return "Bolo, ki password save karna hai?"
        p = load_pass()
        p["main"] = pwd
        save_pass(p)
        return "Ok, maine password yaad kar liya."
    if "what is my password" in q or "mera password" in q or "my password" in q:
        p = load_pass()
        if "main" in p:
            return f"Your password is {p[main]}."
        return "Sorry, koi password save nahi hai."
