...
def is_personal_query(q):
    for w in ["my password", "mera password", "amar password", "password kya", "password ki", "password bolo", "show password", "what is my password", "password batao", "pwd"]:
...
def open_app_by_name(app_name):
    ...
    return "Sorry, Android is blocking me from opening " + clean + " in the background. APK will fix this."

def check_info(q):
    if any(x in q for x in ["time", "samay", "somoy", "waqt", "baje", "bajay", "kya time"]):
        return "अभी " + get_time() + " बजे हैं।", "hi"
    # Weather checked BEFORE date to avoid conflict
    if any(x in q for x in ["weather", "temperature", "mausam", "garmi", "sardi", "tapman", "barish"]):
        w = get_weather()
        if w: return "आज का मौसम: " + w, "hi"
        return "क्षमा करें, मौसम की जानकारी नहीं मिल रही।", "hi"
    if any(x in q for x in ["date", "today", "aaj", "tarikh", "kya din"]):
        return "आज " + get_date() + " है।", "hi"
    return None, None
...
while True:
    ...
    if any(x in q for x in ["bye", "by", "bey", "stop", "band"]):
        bolo("Bye boss!")
        break
    ...
    if any(x in q for x in ["personal mood", "personal mode", "private mode"]):
        personal_mode = True
        bolo("Personal mood on.", "en")
        continue
    if any(x in q for x in ["normal mood", "normal mode", "public mode"]):
        personal_mode = False
    ...
    if personal_mode:
        ...
        elif "time" in q or "samay" in q: bolo("अभी " + get_time() + " बजे हैं जान।", "hi")
