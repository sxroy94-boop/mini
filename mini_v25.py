...
def normal_reply(q, n):
    if any(x in q for x in ["time", "samay", "somoy", "waqt", "baje", "bajay"]):
        return "अभी " + get_time() + " बजे हैं।"
    if any(x in q for x in ["date", "today", "aaj", "tarikh"]):
        return "आज " + get_date() + " है।"
...
while True:
    ...
    if personal_mode:
        if any(x in q for x in ["hello", "hi"]): bolo("हेलो जान! कैसे हो?", "hi")
    ...
    if rep:
        print("Mini:", rep)
        bolo(rep, "en" if not any(c in rep for c in "अआइईउऊएऐओऔकखगघचछजझटठडढणतथदधनपफबभमयरलवशषसह") else "hi")
        continue
