...
def ask_wiki(q):
    query = q
    for kw in ["who is", "what is", "search for", "search", "google", "kya hai", "kaun hai"]:
        if kw in q:
...
while True:
    ...
    if is_personal(q):
        bolo("Sorry, personal kotha baire pathai na.", "en")
        continue

    if any(x in q for x in ["search", "google", "who is", "what is", "kya hai", "kaun hai"]):
        res = ask_wiki(q)
        if res:
            print("Mini:", res)
            bolo(res, "en")
        else:
            bolo("Sorry, ami ei proshner uttor jani na.", "en")
        continue
