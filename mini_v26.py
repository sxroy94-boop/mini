...
while True:
    ...
    if is_personal_query(q):
        p = lp()
        if "main" in p: bolo(f"Your password is {p['main']}", "en")
    ...
    if personal_mode:
        if any(x in q for x in ["hello", "hi"]): bolo("Hello jaan! Kaise ho?", "hi")
        elif "love" in q: bolo("I love you too jaan.", "hi")
        elif any(x in q for x in ["kaise", "kemon"]): bolo("Main theek hoon jaan.", "hi")
        elif any(x in q for x in ["thank", "thanks"]): bolo("Aww, koi baat nahi jaan!", "hi")
        elif "time" in q or "samay" in q: bolo("Abhi " + get_time() + " bajay hain jaan.", "hi")
        else: bolo("Bolo jaan, main sun rahi hoon.", "hi")
        continue

    # TIME / DATE / WEATHER FIRST
    info = check_info(q)
    if info:
        print("Mini:", info)
        bolo(info, "en")
