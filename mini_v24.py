...
def normal_reply(q, n):
    ...
    if any(x in q for x in ["good afternoon", "afternoon"]):
        return "गुड आफ्टरनून " + n + "! आपका दिन कैसा गया है?"
    if any(x in q for x in ["good evening", "evening"]):
        return "गुड ईवनिंग " + n + "! आज का दिन कैसा था?"
    if any(x in q for x in ["good night", "night"]):
        return "गुड नाइट " + n + "! स्वीट ड्रीम्स।"
...
while True:
    ...
    if any(x in q for x in ["bye", "stop", "band"]):
        bolo("बाय जान!" if personal_mode else "Bye boss!")
        break
    ...
    if personal_mode:
        if any(x in q for x in ["hello", "hi"]):
            bolo("हेलो जान! कैसे हो?", "hi")
        elif "love" in q:
            bolo("आई लव यू टू जान।", "hi")
        elif any(x in q for x in ["kaise", "kemon"]):
            bolo("मैं ठीक हूँ जान।", "hi")
        elif any(x in q for x in ["thank", "thanks", "dhanyabad"]):
            bolo("अरे, कोई बात नहीं जान!", "hi")
        elif "good night" in q:
            bolo("गुड नाइट माय लव। स्वीट ड्रीम्स।", "hi")
        else:
            bolo("बोलो जान, मैं सुन रही हूँ।", "hi")
        continue
    ...
    if rep:
        print("Mini:", rep)
        bolo(rep, "en" if not any(c in rep for c in ["अ", "आ", "इ", "ई", "उ", "ऊ", "ए", "ऐ", "ओ", "औ", "क", "ख", "ग", "घ", "च", "छ", "ज", "झ", "ट", "ठ", "ड", "ढ", "ण", "त", "थ", "द", "ध", "न", "प", "फ", "ब", "भ", "म", "य", "र", "ल", "व", "श", "ष", "स", "ह"]) else "hi")
        continue
