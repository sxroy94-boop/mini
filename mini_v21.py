...
def is_personal_query(q):
    words = ["my password", "mera password", "amar password", "password kya", "password ki", "password bolo", "show password", "password jante"]
    for w in words:
        if w in q: return True
    return False

def extract_password(q):
    for kw in ["password is", "password is:", "password holo", "password bolo"]:
        if kw in q:
            return q.split(kw)[-1].strip()
    parts = q.split("password")
    if len(parts) > 1:
        return parts[-1].replace("is", "").replace("save koro", "").replace("save kro", "").strip()
    return None
...
while True:
    ...
    if any(x in q for x in ["bye", "stop", "band"]):
        ...
        break
    if "good night" in q:
        bolo("Good night my love. Sweet dreams." if personal_mode else "Good night boss!")
        break
    ...
    # Password save (works in both modes)
    if "save" in q and "password" in q:
        pwd = extract_password(q)
        if pwd:
            p = lp(); p["main"] = pwd; sp(p)
            bolo("Password saved permanently boss.", "en")
        else:
            bolo("Please say the password after 'password is'.", "en")
        continue

    # Password retrieve (works in both modes)
    ...
    if personal_mode:
        # In personal mode, mini replies softer and doesn't do web search
