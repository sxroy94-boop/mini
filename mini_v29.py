...
def check_info(q):
    if any(x in q for x in ["time", "samay", "somoy", "waqt", "baje", "bajay", "kya time"]):
        return "The time is " + get_time() + ".", "en"
    if any(x in q for x in ["date", "today", "aaj", "tarikh", "kya din"]):
        return "Today is " + get_date() + ".", "en"
    if any(x in q for x in ["weather", "temperature", "mausam", "garmi", "sardi", "tapman"]):
        w = get_weather()
        if w: return "The weather is " + w, "en"
        return "Sorry, weather info is not available.", "en"
    return None, None
...
while True:
    ...
    if personal_mode:
        ...
        elif "time" in q or "samay" in q: bolo("The time is " + get_time() + ".", "en")
