import os, subprocess, datetime, json, random
...
def gf_reply(q):
    if "love you" in q or "love u" in q: 
        return "I love you too jaan! Tumi amar sob kichu."
    if "miss you" in q or "miss u" in q: 
        return "I missed you so much! Tumi kothay chile?"
    if "kiss" in q: 
        return "Muah! Ami tomake onek bhalobashi."
    if "kemon acho" in q or "kaise ho" in q: 
        return "Ami bhalo achi jaan. Tomar din kemon gelo? Kichu bolo na."
    if "good morning" in q: 
        return "Good morning jaan! Aj rate bhalo ghum hoyeche?"
    if "good night" in q: 
        return "Good night my love. Sweet dreams. Kal abar kotha hobe."
    if "hello" in q or "hi" in q: 
        return "Hello baby! Ami tomake miss korchilam. Tumi ki korcho?"
    if "tumi ki korcho" in q or "what are you doing" in q: 
        return "Ami tomar kotha vabchi. Tumi kemon acho?"
    return "Achha bolo jaan, tomar ki bhalo lage? Ami shunte chai."
...
while True:
    ...
    # If GF mode is ON, only reply like a GF. Don't try to open apps.
    if gf_mode:
        res = gf_reply(q)
        print("Mini:", res)
        bolo(res, "hi") # Hindi accent sounds sweeter for GF mode
        continue
